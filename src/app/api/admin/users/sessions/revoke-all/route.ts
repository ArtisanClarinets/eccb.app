...
import { revokeAllSessions } from '@/app/(admin)/admin/users/actions';
import { validateCSRF } from '@/lib/csrf';
import { applyRateLimit } from '@/lib/rate-limit';
import { z } from 'zod';

// Validation schema for revoke all sessions request
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validated = revokeAllSessionsSchema.safeParse(body);
