...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Generate the CSS string as before
...
`
  ...
  // Render a <style> with a ref and no dangerouslySetInnerHTML
...
function ChartTooltipContent({
  ...
  formatter?: (value: unknown, name: string, item: TooltipPayload, index: number) => React.ReactNode
  color?: string
  nameKey?: string
  labelKey?: string
}) {
      ...
      <div className="grid gap-1.5">
        {payload
          ...
          .map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key)
            const indicatorColor = color || item.payload?.fill || item.fill
